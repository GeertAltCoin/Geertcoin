...
#if defined(NDEBUG)
# error "Geertcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x01e0ab8f22808260eae3e54c645d1dc137f3e06530b8232d4eee4eb6e4b02065");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Geertcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Geertcoin Signed Message:\n";
...
{
    // Geertcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 24000 blocks, which will occur approximately every 1 month
    nSubsidy >>= (nHeight / 24000); // Geertcoin: 24k blocks in ~41 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60; // Geertcoin: 5 minutes
static const int64 nTargetSpacing = 2.5 * 60; // Geertcoin: 2.5 minutes.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Geertcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Geertcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x01e0ab8f22808260eae3e54c645d1dc137f3e06530b8232d4eee4eb6e4b02065");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "09/Feb/2017 Revolution of the Right is unstoppable in Europe, warns Geert Wilders";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1486880723;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1411067;
        ...
        assert(block.hashMerkleRoot == uint256("0x3891dd68c11ff51a9b6a96fe6e21bd865c867aba10a6291637b7aa450ba90273"));
...
unsigned char pchMessageStart[4] = { 0xf8, 0xd2, 0xe3, 0xbc }; // Geertcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GeertcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GeertcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GeertcoinMiner(CWallet *pwallet)
{
    printf("GeertcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("geertcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GeertcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GeertcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GeertcoinMiner, pwallet));
}
